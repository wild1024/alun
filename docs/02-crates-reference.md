...
带自动 JSON 解析校验的请求提取器（实现 `FromRequest`）。

```rust
async fn create(ValidatedJson(req): ValidatedJson<CreateUserReq>) -> Res<String> { ... }
```
...
| 方法           | 示例输入                 | 输出             |
| ------------ | -------------------- | -------------- |
| `mobile(s)`  | `13812345678`        | `138****5678`  |
| `email(s)`   | `a@b.com`            | `a***@b.com`   |
| `id_card(s)` | `320112199001011234` | `3201****1234` |
| `name(s)`    | `张三丰`                | `张**`          |
...
| `is_url(s)`                | URL 格式  |
| `is_ipv4(s)`               | IPv4 格式 |
| `is_strong_password(s)`    | 强密码     |
| `len_between(s, min, max)` | 长度范围    |
| `is_digits(s)`             | 纯数字     |
...
Mask:{id}_card("320112199001011234");                             // → "3201****1234"
Mask::name("张三丰");                                              // → "张**"
...
Crypto::verify_password("pass123", &hash)?;          // → 验证密码
...
fs.delete("temp/old_file.txt").await?;
let exists = fs.exists("reports/2024/report.pdf").await?;

// FsPlugin 自动管理文件系统生命周期
let fs_plugin = FsPlugin::from_config(&config);
```
